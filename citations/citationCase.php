...
<body>
    ...
    <?php
    $nameSql = "select * from cases where caseNum=$caseId and userId=$userId";
    $nameRes = $mysqli->query($nameSql);
    $nameRow = $nameRes->fetch_assoc();
    $vs = ' vs ';
    $caseName = $nameRow['firstParty'] . $vs . $nameRow['secondParty'];
    ?>
    <h4>
        <center><b><?php echo $caseName ?></b></center>
    </h4><br><br>
    <div class="container" style="display:flex">
        <div class="col" style="flex:1">
            <?php
            $sqlCit = "select * from citation_cases ORDER BY idCit DESC LIMIT 1";
            $resultCit = $mysqli->query($sqlCit);
            if ($resultCit->num_rows > 0) {
                while ($rowCit = $resultCit->fetch_assoc()) {
                    $idCit = $rowCit['idCit'] + 1;
                }
            } else {
                $idCit = 1;
            }
            $sql = "insert into citation_cases(idCit, userid, caseid, citations, remarks) values ($idCit, $userId, $caseId, '$citation', '$remarks')";
            if ($mysqli->query($sql) === FALSE) {
                echo "Error: " . $sql . "<br>" . $mysqli->error;
            } else {
                echo "<h6>Citation Added Successfully</h6>";
            ?>
                <div class="button" style="float: center; width: 30%">
                    <center>
                        <form action="./addToCase.php?caseid=<?php echo $caseId; ?>" method="post">
                            <button name="caseid" style="float: center; left: 8%">Add another citation</button>
                        </form>
                        <br>
        ...
        <div class="col" style="flex:0.5; margin-left:10px">
            <?php
            $sqlCit = "select * from citation_cases where caseid=$caseId and userid=$userId";
            $resultCit = $mysqli->query($sqlCit);
            if ($resultCit->num_rows > 0) {
                $sno = 1;
                ...
                while ($rowCit = $resultCit->fetch_assoc()) {
                    ...
                    $sno++;
                    echo "<td><center>" . $rowCit['citations'] . "</center></td>";
                    echo "<td><center>" . $rowCit['remarks'] . "</center></td></tr>";
                }
                echo "</table></center>";
            } else {
                echo "<center><h6>
                            No citations recorded yet.</h6></center>";
            }
            ?>
        </div>
    </div>
