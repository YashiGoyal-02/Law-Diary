...
<body>
    ...
    $caseId = $_POST['caseid'];
    $todayDate = $_POST['caseDate'];
    $stage = $_POST['stageSelect'];
    // echo $stage;

    $stagesql = "update casedate set stage='$stage' where userid='$userId' and caseid='$caseId' and caseDate='$todayDate'";
    if ($mysqli->query($stagesql) === FALSE) {
        echo "Error: " . $stagesql . "<br>" . $mysqli->error;
    }
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            <?php
                    ...
                    <!-- echo "<td><center>" . date("d-m-Y", strtotime($rowNext['date'])) . "</center></td>"; -->
            <?php
                                    break;
                                }
                            }
