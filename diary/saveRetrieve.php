...
<body>
    ...
    if (isset($_POST['editStage'])) {
        $newStage = $_POST['editStage'];
        // echo $newStage;
    }

    if (isset($_POST['editDate'])) {
        $newNextDate = $_POST['editDate'];
    }


    $oldNextDate = $_POST['currentNext'];
    $magName = $_POST['selectMag'];
    // $nextId = $_POST['nextId'];
    // $nextMag = $_POST['nextMag'];
    // echo $magName;
    ...
    if ($magName == 0) {
        ...
        $magSql = "insert into magistrate (userid,id,name,datemag) values ($userId,$idMag,'$other','$dateNow');";
        if ($mysqli->query($magSql) === FALSE) {
            echo "Error: " . $magSql . "<br>" . $mysqli->error;
        }

        $updateMagSql = "update casedate set magistrateId=$idMag where caseid=$caseid and userid=$userId and date='$todayDate' limit 1";
        echo $updateMagSql;
        if ($mysqli->query($updateMagSql) === FALSE) {
        ...
        // echo "old";
        // echo $todayDate;
        $updateMagSql = "update casedate set magistrateId=$magName where caseid=$caseid and userid=$userId and caseDate='$todayDate' ";
        // $updateMagSql = "update casedate set magistrateId=$magName where id='$caseDateId'";
        // echo $updateMagSql;
        if ($mysqli->query($updateMagSql) === FALSE) {
            echo "Error updating record: " . $mysqli->error;
        }
        // $updateNext = "update casedate set magistrateId=$nextMag where id=$nextId";
        // if ($mysqli->query($updateNext) === FALSE) {
        //     echo "Error updating record: " . $mysqli->error;
        // }
        
    }

    if (isset($_POST['editStage']) && $_POST['editStage'] != NULL) {
        $updateStageSql = "update casedate set stage='$newStage' where caseid=$caseid and userid=$userId and caseDate='$todayDate'";
        // echo  $updateStageSql;
        if ($mysqli->query($updateStageSql) === FALSE) {
            echo "Error updating record: " . $mysqli->error;
        }
        if ($newStage == 'Dismissed') {
            $decideSql = "update cases set isDecided=1, status='Lost' where userid=$userId and caseNum=$caseid";
            if ($mysqli->query($decideSql) === FALSE) {
                echo "Error: " . $decideSql . "<br>" . $mysqli->error;
            }
        } else if ($newStage == 'Allowed') {
            $decideSql = "update cases set isDecided=1, status='Won' where userid=$userId and caseNum=$caseid";
            if ($mysqli->query($decideSql) === FALSE) {
                echo "Error: " . $decideSql . "<br>" . $mysqli->error;
            }
        }
    ...
    if (isset($_POST['editDate']) && $_POST['editDate'] != NULL) {
        $selectSql = "select * from casedate where caseid=$caseid and userid=$userId and caseDate='$todayDate'";
        $resultSql = $mysqli->query($selectSql);
        $rowSql = $resultSql->fetch_assoc();
        $magId = $rowSql['magistrateId'];

        $deleteNDateSql = "delete from casedate where caseid=$caseid and userid=$userId and caseDate > '$todayDate' order by caseDate asc limit 1";
        if ($mysqli->query($deleteNDateSql) === FALSE) {
            echo "Error updating record: " . $mysqli->error;
        }

        $insertNDateSql = "insert into casedate(userid,caseid,caseDate,stage,magistrateId) values ($userId, $caseid, '$newNextDate', '', $magId)";
        if ($mysqli->query($insertNDateSql) === FALSE) {
            echo "Error updating record: " . $mysqli->error;
        }
    }
    ...
    $sql = "select * from cases A join casedate B join magistrate C
        ...
        B.caseDate='$todayDate' and B.magistrateId=C.id;";
        ...
        <td>
            ...
            ?>
                <form action="./editRetrieve.php" method="post" style="display: inline;">
                    <input type="hidden" name="todayDate" value=<?php echo $todayDate ?>>
                    <input type="hidden" name="caseid" value=<?php echo $caseNum ?>>
                    <input type="hidden" name="magNow" value=<?php echo $magIdNew ?>>
