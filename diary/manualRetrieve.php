...
<body>
    ...
    //         B.date='$todayDate' and B.magistrateId=C.id;";

    $sql = "SELECT *,B.id as caseDateId FROM cases A
    JOIN casedate B ON A.userId = B.userid AND A.caseNum = B.caseid
    JOIN magistrate C ON B.userid = C.userid
    WHERE A.userId = '$userId' AND B.caseDate = '$todayDate' AND C.id = B.magistrateId;
    ";
        ...
        <td>
            ...
            ?>
                <form action="./editRetrieve.php" method="post" style="display: inline;">
                    ...
                    <input type="hidden" name="todayDate" value=<?php echo $todayDate ?>>
                    <input type="hidden" name="caseid" value=<?php echo $caseNum ?>>
                    <input type="hidden" readonly name="caseDateId" value=<?php echo $row['caseDateId'] ?>>
