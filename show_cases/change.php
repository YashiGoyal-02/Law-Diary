...
<body>
    ...
    <div class="form1">
        <form action="" method="post">
            <div class="row">
                <div class="col-sm-5">
                    Enter the name of the magistrate:
                    <input type="text" name="mag">
                </div>
                <div class="col-sm-4">
                    Changed On: <input type="date" name="date">
                </div>
            </div>
            <br>
            <div class="row">
    ...
    <?php
    if (isset($_POST['mag']) && isset($_POST['date'])) {
        $magistrate = $_POST['mag'];
        $date = $_POST['date'];

        $getMag = "select * from magistrate where userid=$userId and caseid=$id order by id desc limit 1";
        ...
        $magistratesql = "insert into magistrate (userid, caseid, id, name, datemag) values ('$userId', '$id', '$idMag', '$magistrate')";
        if ($mysqli->query($datesql) === FALSE) {
            echo "Error: " . $datesql . "<br>" . $mysqli->error;
        }


        $sql = "UPDATE casedate SET magistrateId=$idMag WHERE caseid=$caseId and userid=$userId and datemag>$date";
        if ($mysqli->query($sql) === TRUE) {
            echo "<br><br><br><h4 style='color: grey'><center>Case Details Updated Successfully.</center></h4>"; ?>

            <br><br>
            <div class="button" style="float: center; width: 30%">

                <form action='../home.php' class='button-3' style='display: inline; font-size:23px'>
                    <input type='submit' value='Go to Home Page'>
                </form>
                <br><br>
                <form action='./case_view.php' class='button-3' style='display: inline; font-size:23px'>
                    <input type='submit' value='View Case List'>
                </form>
